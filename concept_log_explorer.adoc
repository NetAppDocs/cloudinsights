...
3. We have zoomed in on the graph to show only log results that occurred within a specific time frame. The time range shown here reflects the current zoom level. Select the _Reset Zoom_ button to set the zoom level back to the current Cloud Insights time range.

4. We have chosen to display the chart Groued By the "source" field. The chart shows results grouped by color. Hovering over a column in the chart will display some details about the specific entries.
+
