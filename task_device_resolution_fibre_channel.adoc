...
Devices that have been resolved by manual resolution contain a status of _OK_ and identify the rule used to identify the device. Missing devices have a status of _Unidentified_. The total coverage for identification of devices is listed on this page.
...
You can manually import fibre channel device identification into Cloud Insights Automatic Device Resolution feature using a list of devices in a .CSV file.

. Before you begin

You must have a correctly formatted .CSV file in order to import device identifications directly into the Automatic Device Resolution feature. The .CSV file for fibre channel devices requires the following information:
