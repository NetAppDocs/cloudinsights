...
[.lead]
Cloud Insights uses the Dell EMC PowerScale (previously Isilon) SSH data collector to acquire inventory and performance data from PowerScale scale-out NAS storage. Please note that the PowerScale / Isilon REST API based collector has superior functionality and ease of deployment versus this collector type, and NetApp recommends switching to usage of that collector type.
