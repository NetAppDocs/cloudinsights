...
To analyze Infrastructure Changes, navigate to *Observability > Change Analysis*. Alerts and Configuration changes for the current time range are listed. Alert rows show the current alert status (active or resolved). Similarly, configuration changes show the current status of the change.
...
The following logs are monitored for changes:

* logs.infrastructure.changes
* logs.netapp.ems

Metric monitors are also watched for objects and attributes in the list below. In the monitor, these must be selected in the _Group by_ option in order for Change Analytics to track them.
