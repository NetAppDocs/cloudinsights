...
----
[root@ci-eng-ljian-linau bin]# ./securityadmin -pr
...
For example, to remove the proxy, just run this command:

 [root@ci-eng-ljian-linau bin]# ./securityadmin -pr -rp
 proxy command line arg is -rp
