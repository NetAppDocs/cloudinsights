...
In the AWS Data Collector *Advanced Configuration* section, you can set the *Include extra regions* field to include additional region data. By default, this field is set to _"us-.*"_, which collects on all US AWS regions.  To collect on _all_ regions, set this field to  _".*"_. 
