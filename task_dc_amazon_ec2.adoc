...
|Tag Keys and Values on which to Filter VMs|Choose which VMs (and associated disks) to include/exclude by filtering for keys and values that match keys and values of tags on the VM. The Tag Key is required. The Tag Value is optional.
