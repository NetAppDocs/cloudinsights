...
|Choose 'Exclude' or 'Include' to Apply to Filter VMs by Tags|You can choose to filter specific VMs in or out of polling. 
|Tag Keys and Values on which to Filter VMs|Click *+Device* to choose which VMs (and associated disks) to include/exclude by filtering for keys and values that match keys and values of tags on the VM. Tag Key is required. Tag Value is optional.
