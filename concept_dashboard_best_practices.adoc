...
Cloud Insights acquires counters and metrics using names that sometimes differ from data source to data source.
...
*Best practice*: Use the search feature in fields such as Filter by or places like the column selector to find what you are looking for. For example, searching for "cap" will show all metrics with "capacity" in their names, no matter where they occurs in the list. You can then easily select the metrics you want from that shorter list.
...
In dashboards and landing pahes, the asset type around which you are building your widget determines the other asset type counters for which you can filter or add a column. Keep the following in mind when building your widget:
...
+
image:tbd.png[ScatterPlot example 1]
...
+
image:tbd.png[ScatterPlot example 2]
