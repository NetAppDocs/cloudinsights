...
== Running Annotation Rules in Large Environments

In a large Cloud Insights environment, you may notice that running annotation rules seems to take a while to complete. This is because changes to annotation rules cause the Cloud Insights indexer to update. The indexer is what gives Cloud Insights the ability to search or filter for new or updated objects and counters in your data. The rules engine waits until the indexer completes its update before applying the rules.
