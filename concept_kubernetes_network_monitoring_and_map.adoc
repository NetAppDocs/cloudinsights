...
Note that Cloud Insights fetches up to 1,000 alerts for a single workload in the slideout detail panel, as well as 1,000 alerts for the map as a whole.
...
The following metrics are shown for workload maps. Note that these metrics may change over time.

|===
|Metric Name|Description
|bytes_total|Sum total of transmitted bytes and received bytes
|connections_total|
|Average number of  active connections within aggregation interval between a source and destination.
|latency_rtt|Average RTT across all TCP calls between the given pair of source and destination
|rx_bytes_per_sec|Received bytes per second
|tcp_jitter|Average variation of rtt between a given source and destination
|tcp_retransmit_bytes|Total number of bytes retransmitted
|tcp_retransmits_percent|Percentage of retransmitted bytes
|throughput|bytes_total per second
|tx_bytes_per_sec|Transmitted bytes per second
|===
