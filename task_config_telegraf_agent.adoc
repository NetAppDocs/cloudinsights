...
.Steps to install an agent on Kubernetes:

image:AgentInstallKubernetes.png[Kubernetes Agent Install]

.Pre-requisites:

* The `curl` and `sudo` commands must be available. 
* You must have at least one Agent Access Key configured. 
...
To verify that the access token works as needed, run the following to confirm the kubelet API is accessible:
 ...
 curl -v -X GET -H "Authorization: Bearer $TOKEN" https://INSERT_KUBELET_ADDRESS/stats/summary
