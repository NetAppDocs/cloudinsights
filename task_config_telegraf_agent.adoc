...
NOTE: If you wish to perform verifications on the downloaded files before installing the agent, see <<I want to Download the Agent, Verify Checksums, and then Install>> [*these instructions*].
...
* Operator-based configuration. This is recommended for ONTAP data collectors.
...
== I want to Download the Agent, Verify Checksums, and then Install

The Cloud Insights agent installer performs its own integrity checks, but some users may want to perform their own verifications before installing or applying downloaded artifacts. To perform these checks before installing the Cloud Insights agent, these users can edit the agent installation command obtained from the UI and remove the trailing “install” option.
...
This will download all required artifacts from Cloud Insights to the working directory.  The artifacts include, but may not be limited to: 

* an installation script
* an environment file
* YAML files
* a signed checksum file
* a PEM file for signature verification

The installation script, environment file, YAML files, and PEM file can be inspected and verified visually. The signed checksum file can be verified by running the following:
...
Windows:
 ...
 Import-Certificate -Filepath netapp_cert.pem  -CertStoreLocation cert:\CurrentUser\Root
 Get-AuthenticodeSignature sha256.signed.ps1
