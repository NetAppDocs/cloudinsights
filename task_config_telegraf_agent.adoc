...
E0901 15:22:09.827927 1 reflector.go:178] k8s.io/kube-state-metrics/internal/store/builder.go:352: Failed to list *v1.ValidatingWebhookConfiguration: the server could not find the requested resource
E0901 15:22:10.894959 1 reflector.go:178] k8s.io/kube-state-metrics/internal/store/builder.go:352: Failed to list *v1.VolumeAttachment: the server could not find the requested resource
...
to get kube-state-metrics version:

_kubectl get deploy/kube-state-metrics -o jsonpath='{..image}'_

To prevent these from happening, users can modify their kube-state-metrics deployment to disable the leases _mutatingwebhookconfigurations_, _validatingwebhookconfigurations_, and _volumeattachments resources_. More specifically, they can use the following CLI argument:
