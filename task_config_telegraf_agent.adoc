...
* if you are running a kubernetes variant that requires security context constraints, follow the instructins in the *Configuring the Agent to Collect Data from Kubernetes* section.
...
After the agent is installed, generate the Telegraf DaemonSet YAML and ReplicaSet YAML using the following commands. Note that these commands are using the default namespace "ci_monitoring". 
