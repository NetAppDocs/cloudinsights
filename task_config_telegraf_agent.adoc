...
* Remove the current the telegraf services
+
 kubectl --namespace monitoring delete ds telegraf-ds
 kubectl --namespace monitoring delete rs telegraf-rs

* Install link:Kubernetes[the new agent].
...
=== Upgrading the Telegraf Agent on Windows
...
* Install the new telegraf agent
+




=== Upgrading the Telegraf Agent on RHEL/CentOS
...
=== Upgrading the Telegraf Agent on Debian/Ubuntu
...
=== Upgrading the Telegraf Agent on macOS
...
RHEL an CentOS
•	Run `systemctl stop telegraf` or `/etc/init.d/telegraf stop`
•	Run `yum remove telegraf`
•	Follow “Steps to install agent on RHEL/CentOS”

Ubuntu and Debian
•	Run `systemctl stop telegraf` or `/etc/init.d/telegraf stop`
•	Run `dpkg -r telegraf`
•	Follow “Steps to install agent on Ubuntu/Debian”

macOS
•	Run `sudo launchctl stop telegraf`
•	Run cp /Applications/telegraf.app/scripts/uninstall /tmp
•	Run sudo /tmp/uninstall
•	Follow “Steps to install agent on macOS”

Windows
•	Run `Stop-Service telegraf`
•	Run `sc.exe delete telegraf`
•	Remove the SYSTEM\CurrentControlSet\Services\EventLog\Application\telegraf key from the registry
•	Remove C:\Program Files\telegraf\telegraf.conf
•	Remove C:\Program Files\telegraf\telegraf.exe
•	Follow “Steps to install agent on Windows”
