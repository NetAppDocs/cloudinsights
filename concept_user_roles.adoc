...
The following image is from a user currently logged in with Administrator access to Cloud Secure.  The user being invited is being given Administrator permissions in Cloud Secure and Guest permissions in Monitor and Optimize. Since the current administrator does not have access to Reporting, they cannot assign any permissions to Reporting for the new user.
+
