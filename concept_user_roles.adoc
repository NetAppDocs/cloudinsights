...
To configure _SSO User Auto-Provisioning_, on the *Admin > User Management* page, You must first have set up BlueXP Identity Federation. Select the *Set Up Federation* link in the banner to proceed to BlueXP Federation. Once that is configured, Cloud Insights administrators can then enable SSO user login. When an administrator enables _SSO User Auto-Provisioning_, they choose a default role for all SSO users (such as Guest or User). Users who log in through SSO will have that default role.
