...
In addition to inviting users, administrators who are federated for their corporate domain environment can enable *Single Sign-On* (SSO) access for all users in that domain, without having to invite them individually. With SSO enabled, any user with the domain email address can log into Cloud Insights.
...
Occasionally, an administrator will want to promote a single user out of the default SSO role (for example, to make them an administrator). They can accomplish this on the *Admin > User Management* page by clicking on the "three-dot" menu for the user and selecting *Assign Role*.

NOTE: Users who are assigned an explicit role in this way will continue to have access to Cloud Insights even if SSO is disabled.
