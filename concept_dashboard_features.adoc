...
Most widgets on a dashboard allow you to specify the Units in which to display values, for example, KB/s, Thousands, Dollars, etc. In many cases, Cloud Insights knows or can guess the best format for the data based on the metadata (data about the data) that is being ingested.
