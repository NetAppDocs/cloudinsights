...
In this example, the data selected for the widget is known to be IO Operations per Second (IOPS), so the Base Unit is autommatically selected as 'IO/s'. The Y-axis on the graph shows 'IO/s' is the base unit, and all values are displayed in terms of that base unit.
