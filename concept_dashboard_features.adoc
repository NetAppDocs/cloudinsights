...
Dashboards are configured to automatically refresh widgets at a time interval determined by the Dashboard Time Range selected. By default, the dashboard time range is 24 hours, which corresponds to an auto-refresh interval of five minutes.
...
Auto-Refresh will occur for every widget based on the chosen interval, regardless of whether that widget is configured to override dashboard time.
