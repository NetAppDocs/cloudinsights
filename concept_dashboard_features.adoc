...
The Solid Gauge, Bullet Gauge, and Single-Value widgets allow you to set values for _Warning_ and/or _Critical_ levels, which determine the color in which the widget displays. Additionally, in the Single-Value widget, you can choose to have "In Range" values (those below _Warning_ level) shown with either green or white background.
...
You can optionally choose a minimum and maximum value for the gauge. Values below minimum will not display the gauge. Values above maximum will display a full gauge. If you do not choose minimum or maximum values, the widget selects optimal min and max based on the value.
