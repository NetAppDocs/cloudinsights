...
Note that only Attribute variables, and Annotations of type _Text_, _Boolean_, or _List_, can be filtered automatically using variables like this. _Non-text_ annotations such as numbers or dates cannot be automatically filtered, nor can variables other than Annotation or Attribute type (i.e. Number, Date, Boolean). Individual widgets must each be configured to use variables of these types.
