...
* *Count*: displays a _count_ of objects that have reported data within the specified time frame. You can choose the _Entire Time Window_ as determined by the dashboard time range setting (or widget time range, if set tyo override the dashboard time), or a _Custom Time Window_ that you select.
