...
//* *Attribute*: this includes annotations, attributes, and metrics 
...
==== Setting a non-attribute variable
...
By default, when you add an Annotation variable to your dashboard, the variable is applied to _all_ widgets on the dashboard, meaning that all widgets on your dashboard where the variable is relevant will display results filtered by the value you set in the variable. Note that _only_ Annotation-type variables can be applied universally like this. Other variable types must be set in individual widgets.

image:Variables_Automatic_Filter_Button.png[Automatic Filter]

To change this so that the variable only applies to the widgets where you have specifically set the variable, click the "Filter automatically" slider to disable it.

To set a variable in an individual widget, open the widget in edit mode and select specific annotation in the _Filter By_ field. With an Annotation variable, you can select one or more specific values, or select the Variable name (indicated by the leading "$") to allow typing in the variable at the dashboard level. Only those widgets for which you set the variable will show the filtered results.
...
* Cannot contain any special characters; only the letters a-z, the digits 0-9, period (.), and underscore (_) are allowed.
