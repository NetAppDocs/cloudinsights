...
== Displaying Values in Different Units

Most widgets on a dashboard allow you to specify the Units in which to display values, for example IO/s, Thousands, Percentage, Milliseconds (ms), etc. In many cases, Cloud Insights knows the best format for the data based on the metadata (data about the data) that is being ingested. In cases where the best format is not known, you can set the format you want.

In the example below, the data selected for the widget is known to be input/output operations per second (IOPS), so the Base Unit is autommatically selected as 'IO/s'. The Y-axis on the graph shows 'IO/s' is the base unit, and all values are displayed in terms of that base unit.

image:WidgetBaseUnitKnown.png[Widget with known base unit]

In this example, the data is displayed in Whole Number format. You can choose another format in which to display the values. Since the base unit is a simple numeric in this case, you can choose from among the supported numerical formats: whole number, thousand, million, billion, or trillion. The Y-Axis label and values change according to the format you choose, as shown below.

image:WidgetBaseUnitKnown-Formatted.png[Choosing to display values as 'thousand']

In cases where the base unit is not known, you can assign a unit from among the available units, or type in your own. 
...
||pebibyte|PiB
||exbibyte|EiB
|Data (Metric)|kilobyte|KB
||megabyte|MB
||gigabyte|GB
||terabyte|TB
||petabyte|PB
||exabyte|EB
|Data Rate (IEC)|bit/sec|b/s
||byte/sec|B/s
||kibibyte/sec|KiB/s
||mebibyte/sec|MiB/s
||gibibyte/sec|GiB/s
||tebibyte/sec|TiB/s
||pebibyte/sec|PiB/s
|Data Rate (Metric)|kilobyte/sec|KB/s
||megabyte/sec|MB/s
||gigabyte/sec|GB/s
||terabyte/sec|TB/s
||petabyte/sec|PB/s
