...
By default, when you add an Annotation variable to your dashboard, the variable is applied to _all_ widgets on the dashboard, meaning that all widgets on your dashboard where the variable is relevant will display results filtered by the value you set in the variable. 

image:Variables_Automatic_Filter_Button.png[Automatic Filter]

Note that _only_ text-based Annotation-type variables can be applied universally like this. Non-text Annotations such as number or date cannot be automatically filtered, nor can non-Annotation variables (Number, Date, Boolean). These variables must be configured in individual widgets.
...
To set a variable in an individual widget, open the widget in edit mode and select the specific annotation in the _Filter By_ field. With an Annotation variable, you can select one or more specific values, or select the Variable name (indicated by the leading "$") to allow typing in the variable at the dashboard level. Only those widgets for which you set the variable will show the filtered results.
