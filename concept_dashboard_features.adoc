...
The Solid Gauge, Bullet Gauge, and Single-Value widgets allow you to set values for _Warning_ and/or _Critical_ levels, which determine the color in which the widget is displayed. Additionally, in the Single-Value widget, you can choose to have "In Range" values (those below _Warning_ level)shown with either green or white background.

image:Gauge Widget Formatting.png[Format settings for Gauge Widget]

To set formatting for these widgets, first choose whether to highlight values above or below your thresholds. In this example, we want to highlight values greater than (>) the threshold levels.

Choose a value for the "Warning" level. When the widget displays values above this level, it displays the gauge in amber. 

Choose a value for the "Critical" level. Values above this level will cause the gauge to display in red.

You can optionally choose a minimum and maximum value for the gauge. Values below minimum will not display the gauge. Values above maximum will display a full gauge. If you do not choose minimum or maximum values, the widget will select optimal min and max based on the value.

image:Single-value and Gauge Widgets.png[Single-Value and Gauge Widgets]
