...
* *Annotation*: Use a pre-definied link:task_defining_annotations.html[Annotation] to filter widget data.
...
To set a variable in a specific widget, open the widget in edit mode and select specific annotation in the _Filter By_ field. With an Annotation variable, you can select one or more specific values, or select the Variable name (indicated by the leading "$") to allow typing in the vairable at the dashboard level. Only those widgets for which you set the variable will show the filtered results.
