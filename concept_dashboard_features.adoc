...
When you select a value or values for a field, the other filters for that query will show values relevant to that filter.
...
Note that  only Text filters will show context. Date, Enum (list), etc. will not filter out. The exception to this is that you can set a filter on an Enum (i.e. list) field and have other text fields be filtered in context. In this case, the list fields acts as a source for the remaining fields. For example, selecting a value in an Enum field like Data Center, then other filters will show only the models/names in that data center)
