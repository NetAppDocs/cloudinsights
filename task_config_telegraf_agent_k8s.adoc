...
= Configuring an Agent on Kubernetes to Collect Data
...
IMPORTANT: This page is currently in progress.  For the most up-to-date instructions for installing an agent on Kubernetes, link:task_config_telegraf_agent.html[*go here*].
...
NOTE: If you want to verify the installation files before instaling the Agent, read about <<Verifying Checksums>>.
...
To install an agent on Kubernetes, you must first do the following:
