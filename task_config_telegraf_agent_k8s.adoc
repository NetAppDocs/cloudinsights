...
If the cluster you are monitoring contains Custom Resources which do not have a ClusterRole which aggregates to view \https://kubernetes.io/docs/reference/access-authn-authz/rbac/#aggregated-clusterroles, you will need to manually grant the operator access to these resources to monitor them with Change Management or Event Logs.
