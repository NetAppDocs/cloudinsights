...
If you use a proxy in your environment, in order to install the NetApp Kubernetes Operating Monitor, you must first ensure that your proxy is configured to allow good communication to your Cloud Insights environment. If you have a proxy and can access Cloud Insights from the server/VM on which you wish to install the Operator, then your proxy is likely configured properly.
...
To set the variable(s), perform the following steps on your system *PRIOR* to installing the NetApp Kubernetes Monitoring Operator:
...
<<installing-the-netapp-kubernetes-monitoring-operator, Install>> the NetApp Kubernetes Monitoring Operator.
