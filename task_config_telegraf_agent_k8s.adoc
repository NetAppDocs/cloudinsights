...
If the cluster you are monitoring contains Custom Resources which do not have a ClusterRole which link:https://kubernetes.io/docs/reference/access-authn-authz/rbac/#aggregated-clusterroles[aggregates to view], you will need to manually grant the operator access to these resources to monitor them with Change Analytics or Event Logs.  

. Edit _operator-additional-permissions.yaml_ before installing or after installing edit the resource ClusterRole/netapp-ci-<namespace>-additional-permissions 
. Create a new rule for the desired apiGroups and resources with the verbs "get", "watch", "list". See link:https://kubernetes.io/docs/reference/access-authn-authz/rbac[the Kubernetes documentation] for more information
