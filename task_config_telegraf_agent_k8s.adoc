...
Below is a high-level illustration showing where the Operator resides in your environment. Depending on your environment, _Proxy Server" may or may not be required. 
...
The Operator (NKMO) and the data collectors are downloaded from the Cloud Insights Docker Registry. Once installed, NKMO then manages the collectors which are deployed in each node of the Kubernetes cluster to acquire data, including managing the life cycle of those collectors. The collectors . Following this chain, data is acquired from the the collectors and sent through to Cloud Insights. 
