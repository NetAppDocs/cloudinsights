...
These components are enabled by default; if you decide you don't need a component, you can disable it by navigating to *Kubernetes > Collectors* and selecting _Modify Deployment_ from a collector's "three dots" menu on the right of the screen.

image:KubernetesModifyDeploymentMenu.png[Modify deployment Menu on Kubernetes Collector list page]

The screen shows the current state of each component and allows you to disable or enable comnponents for that collector as needed. 
