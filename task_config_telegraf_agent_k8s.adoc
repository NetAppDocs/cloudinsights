...
|I see gaps in my timeseries widgets and I see my _netapp-ci-telegraf-rs_ pod reporting warnings of "[inputs.prometheus] Collection took longer than expected; not complete after interval of 1m0s" 
|kube-state-metrics is likely experiencing difficulty exporting all of the kubernetes metrics in a timely fashion. To alleviate this, try increasing the number of kube-state-metrics shards.  for more details, refer to the operator's <<telegraf_agent_k8s_config_options.html,configuration options>>.
