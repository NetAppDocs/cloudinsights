...
NOTE: If you are monitoring Kubernetes in your Cloud Insights environment prior to October 12, 2023 and are upgrading the NetApp Kubernetes Monitoring Operator, you must <<uninstalling,uninstall>> the existing NetApp Kubernetes Monitoring Operator before installing the latest. Changes were made to rename the Kubernetes objects laid down by the NetApp Kubernetes Monitoring Operator in order to facilitate users who are looking to deploy similar applications in the same namespace.
...
To stop the Netapp Kubernetes Monitoring Operator:
 ...
 kubectl -n netapp-monitoring scale deploy netapp-ci-monitoring-operator --replicas=0

To start the Netapp Kubernetes Monitoring Operator:

 kubectl -n netapp-monitoring scale deploy netapp-ci-monitoring-operator --replicas=1
...
If a Security Context Constraint was previously-created:
 ...
 kubectl delete scc netapp-ci-<NAMESPACE>-telegraf-hostaccess 
...
Edit the netapp-ci-monitoring-operator deployment  deployment in operator-deployment.yaml, and modify all image references to use your private Docker repository.
...
|I am having an issue with my netapp-ci-monitoring-operator deployment, and the current documentation does not help me resolve it.
...
----
 ...
 kubectl -n netapp-monitoring logs <netapp-ci-monitoring-operator-pod> --all-containers=true
 kubectl -n netapp-monitoring logs <telegraf-pod> --all-containers=true
----

|netapp-ci-net-observer (Workload Map) pods in NKMO namespace are in CrashLoopBackOff
...
•	Netapp-ci-net-observer pods requires the Linux kernel version to be at least 4.18.0. Check the kernel version using the command “uname -r” and ensure they are >= 4.18.0

|netapp-ci-net-observer pods in NKMO namespace are in CrashLoopBackOff in OpenShift 4 environment
...
|Some of the netapp-ci-net-observer pods in NKMO namespace are in Pending state
|netapp-ci-net-observer is a DaemonSet and runs a pod in each Node of the k8s cluster.
