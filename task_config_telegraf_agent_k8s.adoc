...
. Log into the host use will use for agent installation.  It should have _kubectl_ configured to communicate with the target K8s cluster, and have Internet connectivity to your Cloud Insights environment.
...
>> This code block needs to be commented (Pranav?)
...
* Make note of your cluster name.  You can view the cluster name by running one of the following commands:
...
* <<netApp-kubernetes-monitoring-operator-installation, Install>> the new agent].
...
<<netApp-kubernetes-monitoring-operator-installation, Install>> the current Operator. Use the same cluster name, and ensure you are pulling new container images if you have set up a custom repo.
