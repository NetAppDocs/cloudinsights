...
If you are running with a <<configuring-proxy-support,proxy>>, have a <<using-a-custom-private-docker-repository,custom repository>>, or are using <<openshift-instructions,OpenShift>>, read the following sections carefully.
...
To configure a proxy, perform the following steps after you install the NetApp Kubernetes Monitoring Operator.
...
>> This code block should be commented:
...
If using a custom docker repository, do the following after installing the NetApp Kubernetes Monitoring Operator:
...
Openshift may implement an added level of security that may block access to some Kubernetes components. The _SecurityContextConstraint_ is not created as part of the Kubernetes agent install command provided in the Cloud Insights UI, and must be created manually. See the <<configuring-the-agent-to-collect-data-from-kubernetes,Configuring the Agent to Collect Data from Kubernetes>> section for more information.
...
* Make note of your cluster name.  You can view the cluster name by running the following command. If your namespace is not the default (_ci-monitoring_), substitute the appropriate namespace:
...
* <<netApp-kubernetes-monitoring-operator-installation, Install>> the new Operator.
...
* <<netApp-kubernetes-monitoring-operator-installation, Install>> the current Operator. 
...
* Make note of your cluster name.  You can view the cluster name by running the following command. If your namespace is not the default (_netapp-monitoring_), substitute the appropriate namespace:
...
<<to-remove-the-netApp-kubernetes-monitoringoOperator, Uninstall>> the current Operator.

<<netApp-kubernetes-monitoring-operator-installation, Install>> the Operator. Use the same cluster name, and ensure you are pulling new container images if you have set up a custom repo.
...
NOTE: If you have a custom repository, you must follow the instructions for <<using-a-custom-private-docker-repository, Using a custom/private docker repository>>.
...
NOTE: If you are running on a previously-installed script-based Kubernetes agent, you must <<upgrading-the-agent, upgrade>> to the NetApp Kubernetes Monitoring Operator.
