...
* kube-state-metrics is essential, and is automatically installed with the NetApp Kubernetes Monitoring Operator. Note that kube-state-metrics version 2.0 and above does not export Kubernetes object labels by default; you must configure kube-state-metrics to export these. <<about-kube-state-metrics, Read more>>.
////
