...
Cloud Insights has built a Kubernetes Operator for monitoring customers' Kubernetes clusters.  This "NetApp Kubernetes Monitoring Operator" (NKMO) is an improvement over script-installed monitoring methods because it allows more flexible configuration of monitoring controllable from Cloud Insights, fewer customer-driven configuration interventions, and enhances opportunities for monitoring other software running in the K8s cluster.
