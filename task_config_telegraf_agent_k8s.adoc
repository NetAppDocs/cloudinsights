...
== Tuning the NetApp Kubernetes Monitoring Operator

You can adjust the NetApp Kubernetes Monitoring Operator for optimal performance by fine-tuning certain variables for either the Custom Resources or the Telegraf agent.  See the following tables for variables that you can set.
...
|DS_CPU_LIMITS_PLACEHOLDER	|agent	|yes	|k8 CPU limit for telegraf-ds
|DS_MEM_LIMITS_PLACEHOLDER	|agent	|yes	|k8 mem limit for telegraf-ds
|DS_CPU_REQUEST_PLACEHOLDER	|agent	|yes	|k8 cpu requests for telegraf-ds
|DS_MEM_REQUEST_PLACEHOLDER	|agent	|yes	|k8 memory requests for telegraf-ds
|RS_CPU_LIMITS_PLACEHOLDER	|agent	|yes	|k8 CPU limit for telegraf-rs.
|RS_MEM_LIMITS_PLACEHOLDER	|agent	|yes	|k8 mem limit for telegraf-rs
|RS_CPU_REQUEST_PLACEHOLDER	|agent	|yes	|k8 cpu requests for telegraf-rs
|RS_MEM_REQUEST_PLACEHOLDER	|agent	|yes	|k8 memory requests for telegraf-rs
|KSM_CPU_REQUEST_PLACEHOLDER:	|ksm	|yes	|k8 cpu requests for kube-state-metrics deploy
|KSM_MEM_REQUEST_PLACEHOLDER:	|ksm	|yes	|k8 cpu requests for kube-state-metrics deploy

|===

=== Misc/security variables
...
|INSECURE_SKIP_VERIFY_PLACEHOLDER	|(telegraf-plugins)	|no	|disable ssl verification for telegraf output
|===
...
|GLOBAL_FLUSH_INTERVAL	|agent	|no	|(sets telegraf flush_interval, type interval): Default time telegraf will wait between writing outputs.
|FLUSH_JITTER	|agent	|no	|(sets telegraf flush_jitter, type interval): Each output will wait a random amount of time between the scheduled write time and that time + flush_jitter before writing outputs
