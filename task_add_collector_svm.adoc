...
** SMB protocol version 3.1 and earlier.  Note that Cloud Secure does not work with SMB configurations that use Flexcache. Fpolicy does not support an SMB+FlexCache environment.

Please see the FlexCache document. (Starting in ONTAP9.7, Fpolicy is supported only NFS environment.)
