...
Keep the following in mind for link:/cloudinsights/cs_restrict_user_access.html[User Access Blocking]:
...
Note: If one rest-role is already added--either _arwrole_ or _csrestrole_--there is no need to add a second rest-role. You can simply add the API permissions as in the example below.

Example: _csrestrole_ is already present, so we only have to enable anti-ransomware protection and give API permissions to the existing _csrestrole_:
