...
* We require read only privileges on all objects in the NetApp HCI Management server. 
...
|===
|Field|Description
|Management server IP Address |IP address of the NetApp HCI Management server
|Management Node IP address|IP address of the NetApp HCI Management node (mNode)
...
|Filter Device List|Comma Separated list of devices for Filtering by ESX_HOST, CLUSTER, and DATACENTER Only
