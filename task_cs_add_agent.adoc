...
* You can read about Cloud Insights security link:security_overview.html[here].
...
By default, Cloud Insights Workload Security updates agents automatically. Some customers may wish to pause automatic updating, which leaves an Agent at its current version until one of the following occurs:

* The customer resumes automatic Agent updates.
* 30 days have passed. Note that the 30 days starts on the day of the most recent previous Agent update, not at the day the Agent is paused.

In each of these cases, the agent will be updated at the next Workload Security refresh.
To pause or resume automatic agent updates, use the _cloudsecure_config.agents APIs_:
...
image:ws_agent_version.png[WS Agent Versions]
