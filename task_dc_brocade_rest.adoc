...
|The Test feature or poll failed with ...Unable to parse JSON response body | CI received a HTTP(S) response, but it is not JSON formatted as we would expect via Brocade FOS REST API. If this is a new collector, are you certain the collector is actually pointed at a FOS device running 8.2 or higher?
