...
== Backing Up AU artifacts

To back up current  artifacts, follow these steps:

.steps
1. Unzip the patch bundle to identify which .jar and .xml files to restore. For example:
...
2. Unzip the DS.war bundle to restore and copy the jar and xml file to folder "/var/lib/netapp/cloudinsights/acq/datasources.war/". Ensure file permissions are correct. For example:
...
3. Restart the Acquisition Unit service and make sure polling is successful. 
