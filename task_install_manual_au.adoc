...
== Backing up Acquisition Unit artifacts

It is recommended to back up your current AU artifacts before applying a patch or update. To back up the current AU artifacts, follow these steps:


. Unzip the patch bundle to identify which .jar and .xml files to restore. For example:
+
----
unzip xp_patch-FSx_for_NetApp_ONTAP_NetApp_ONTAP_netapp_ontap_fail_performance_2023_Apr_12_10_12_38.zip -d temp

 Archive:  xp_patch-FSx_for_NetApp_ONTAP_NetApp_ONTAP_netapp_ontap_fail_performance_2023_Apr_12_10_12_38.zip
  inflating: temp/META-INF/MANIFEST.MF
  inflating: temp/META-INF/NETAPP.SF
  inflating: temp/META-INF/NETAPP.RSA
  inflating: temp/patch.properties
  inflating: temp/netapp_ontap.jar
  inflating: temp/netapp_cmode.xml
  inflating: temp/netapp.svg
  inflating: temp/netapp_aws_fsx.xml
  inflating: temp/fsx.svg
  inflating: temp/metadata.json
  inflating: temp/readme.txt
----

. Unzip the DS.war bundle to restore and copy the jar and xml file to folder "/var/lib/netapp/cloudinsights/acq/datasources.war/". Ensure file permissions are correct. For example:
+
----
unzip datasources-war-1.1676.0.zip -d restore
cd restore
cp netapp_ontap.jar /var/lib/netapp/cloudinsights/acq/datasources.war/
cp netapp_aws_fsx.xml /var/lib/netapp/cloudinsights/acq/datasources.war/
----

. Restart the Acquisition Unit service and make sure polling is successful. 
