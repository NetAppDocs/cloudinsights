...
VMware Tanzu
|Linux OS|Cloud Insights does not support nodes that are running with Arm64 architecture.
...
|Other
|If you are running on OpenShift 4.6 or higher, you must follow the link:task_config_telegraf_agent_k8s.html#openshift-instructions[OpenShift Instructions] in addition to ensuring these pre-requisites are met.
