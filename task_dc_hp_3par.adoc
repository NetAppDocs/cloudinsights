...
|===
|Problem:|Try this:
|Error: "showsys" command doesn't return any result.
|Run "showsys" and "showversion -a" command and check if the version is supported by 3par.
|===
...
|===
|Problem:|Try this:
|Error: Failed to connect or login.  provider initialization failed.
|An all-numeric array name causes problems with SMI-S server. Try changing the array name.
|Error: SMI-S user configured does not have any domain
