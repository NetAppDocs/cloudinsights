...
Cloud Insights introduces *link:concept_dashboard_features.html#auto-refresh-and-tv-mode[TV Mode]*, giving you an uncluttered, *Auto-Refreshing* display on screen for constant, near-real-time viewing of any dashboard or landing page. Data is automatically refreshed according to the dashboard time range selected.
...
A dashboard time range of CUSTOM does not auto-refresh. 
