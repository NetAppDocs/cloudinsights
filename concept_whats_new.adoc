...
The new Change and Change-Ratio time aggregations enable you to view changes in metric values over time, making it easier to identify compelling shifts and trends, such as significant capacity growth.

In addition to last/avg/min/max/sum, you can now aggregate data in charts using:
...
image:change_and_change_ratio_drop_down.png[Drop-down showing change and ca=hange ration aggregation choices]

image:change_and_change_ratio_bar_chart.png[bar chart example showing change and change ration aggregation choices]


////
=== Aggregate by Change or Change Ratio

New rollup aggregation functions give you more flexibility in visualizing your data. In addition to last/avg/min/max/sum, you can now aggregate data in charts using the following:

* *Change* - observe the change in a metric between two points within a selected period
* *Change Ratio* - observe the proportional change in a metric between two points, with respect to the initial point, within a selected period.

image:change_and_change_ratio.png[aggregation drop-down showing change and change ratio]
////

=== Export Log Query Results to .CSV

When viewing log query results, easily export the list to .CSV by clicking the new "Export" button.
...
Cloud Insights now gives you options to resolve alerts with greater accuracy and reliability. Far from alerting on a brief dip or spike values, you can choose to resolve an alert only when the metric stays within the acceptable range for a specified duration. This prevents "flickering" of alerts caused by metric value fluctuations around thresholds, providing peace of mind by reducing unnecessary noise.

image:resolve_alert_by_time_dropdown.png[resolving an alert base don time]
