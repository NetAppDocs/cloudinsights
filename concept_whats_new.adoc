...
On the *Admin > Subscription* and *Help > Support* pages, the field for "Subscription ID" has been renamed to "Subscription Name" for readability.  The data in the field remains as it was.

==== "Owner" field is more readable

In Dashboards, Queries, and on the Audit page, the data for the "Owner" field was previously an authorization ID string, instead of a user-friendly owner name. The "Owner" field now shows a simpler and more readable owner name.
