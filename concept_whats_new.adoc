...
ONTAP Essentials is designed to be intuitive to existing ONTAP operators and administrators, easing the transition from AIQUM to service-based management tools. 
...
Cloud Insights provides two new heartbeat monitors for your Acquisition Units, as well as two monitors to alert you to data collector failures, so you can be alerted to acquisition issues quickly. 

In all, the following monitors are available:
...
Note that the data collector monitors are in _Paused_ state by default. Be sure to activate them when you want to watch for collector issues.
