...
If you have a virtual machine associated with a Kubernetes node, the VM landing page will include a table of the VM's Kubernetes Pods.

image:Kubernetes_Pod_Table_on_VM_Page.png[Kubernetes Pod table on a VM landing page]


==== Page variables now include Attributes 

In addition to Annotation, Text, Date, Number, and Boolean, you can now use an object's Attributes in dashboard variables. 
...
It is now possible to use any Numeric dashboard variable in a widget expression. For example _$NumVar1 * 100_.


==== Add Entitlement ID

If you have a NetApp product that has the Cloud Insights service bundled with it, you can utilize it by adding that product’s serial number to your existing Cloud Insights subscription. For example, if you have purchased NetApp Astra Control Center, the Astra license serial number can be used to identify the subscription in Cloud Insights. Cloud Insights refers to this an _Entitlement ID_. 
...
==== API for Odata in Reporting

***Wording and inclusion TBD****
