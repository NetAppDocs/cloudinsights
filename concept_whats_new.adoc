...
The 2nd and 3rd polls occur within 15 seconds of the previous poll. Polling then proceeds according to the intervals that were set during data collector configuration (typically 15 minutes for performance).

Previously, the initial performance polls took place at the configured (i.e. 15 minute) poll intervals, meaning it would take up to 30 minutes before meaningful data was acquired from a new data collector.
