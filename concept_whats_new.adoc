...
Many of you asked for a dark theme, and Cloud Insights has answered. To switch to dark mode, click the drop-down next to your user name. 
image:DarkModeSwitch.png[Switch to Dark Mode is available in the User drop-down]
image:DarkModeDashboard.png[An image of a typical dashboard shown in dark theme]
 


==== System-Defined Monitors 
Cloud Insights provides a number of default system monitors, comprised of pre-defined metrics and conditions as well as default descriptions and corrective actions. You can find system monitors in the Alerts > Manage Monitors page, in the following Groups:
...
Kubernetes Explorer pages now include filters to allow more precise display of data relevant to your exploration of Kubernetes clusters, nodes, and pods. 
...
Kubernetes data is now delivered to the Cloud Insights Data Warehouse (DWH) through the Extract, Transform, and Load (ETL) process for use in Reporting.
...
The Vplex data collector has been improved, with model and data files split by Vplex cluster.  Because data is now reported with a different originator ID based on serial number instead of IP, historical data will be lost. All new data collected will be based on the new modeling.
