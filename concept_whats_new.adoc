...
Starting April 2, Cloud Insights supports multiple subscriptions for a customer in a single Cloud Insights instance. Managed Units (MUs) from multiple purchased subscriptions are aggregated as long as the subscription serial number is the same. 

Contact NetApp Support for assistance with multiple subscriptions.
