...
=== Improved Kubernetes Detail Page

Cloud Insights makes it easier than ever to explore your Kubernetes environment, with an improved cluster detail page that has added Saturation details as well as a cleaner view into Namespaces and Workloads. 
...
image:KubernetesNodeAge.png[Kubernetes Node list showing age in seconds]


=== What's changed in environment?
...
Cloud Insights is able to prognose the number of days until capacity runs out for each Storage, Storage pool, Internal Volume, and Volume monitored. These values can help to significantly reduce the risk of an outage. 

image:Internal Volume - Time to Full dashboard example.png[Internal Volume TTF forecasting dashboard]

Note that Time-to-Full Forecasting is moving out of _Preview_ and will be rolled out to all customers over time.
