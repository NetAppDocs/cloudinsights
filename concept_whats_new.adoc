...
Cloud Insights will now send out notifications when API tokens are going to expire. Recipients will receive a notification when the token will expire in 30 days or fewer, and again when expiration is imminent. You must rotate expired tokens. Learn more link:API_Overview.html#rotating-expired-api-access-tokens[here].
