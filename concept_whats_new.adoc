...
link:concept_device_resolution_overview.html[*Automatic Device resolution*] identifies devices in your environment that are not identified by the data collectors you configure. 

After you configure data collectors, devices in your environment including switches, storage arrays, and your virtual infrastructure of hypervisors and VMs are identified, you may still have unidentified devices. For these remaining unknown devices, Cloud Insights provides link:concept_device_resolution_overview.html[*Automatic Device resolution*]

Automatic Device Resolution is accomplished through the use of link:task_device_resolution_rules.html[Rules] that you configure to identify devices currently in your environment as well as similar devices as they are added to your environment. Automatic device resolution can help you resolve unknown devices as the following device types:
