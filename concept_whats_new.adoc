...
The following additional time ranges are supported throughout Cloud Insights. On dashboards, the time range selected also determines the refresh interval for widgets. The refresh interval varies based on whether the widget is a time-series widget (line, spline, area, and stacked area charts) or not.
