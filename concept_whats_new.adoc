...
Cloud Secure includes the following recent changes.

*	Better file and folder metadata change recognition to detect if the user changed Permission, Owner, or Group Ownership.
*	Export user activity report to .CSV file. 
* Generate user activity audit reports required for compliance. 
* Use forensic filtering to filter data by any activity attribute such as time, user, volume, path, file type, activity type, etc.

CS monitors and audits all user access operations on files and folders. Activity auditing allows you to comply with internal security policies, meet external compliance requirements such as PCI, GDPR, and HIPAA, and conduct data breach and security incident investigations.
