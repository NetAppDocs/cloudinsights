...
The link:/task_config_telegraf_agent_k8s.html#operator-based-install-or-script-based-install[NetApp Kubernetes Monitoring Operator] (NKMO) is available for all environments running Kubernetes 1.17 or greater and is the preferred method for installing Kubernetes for Cloud Insights. NKMO is an improvement over script-installed monitoring methods because it allows more flexible configuration of monitoring controllable from Cloud Insights and fewer customer-driven configuration interventions, as well as enhances opportunities for monitoring other software running in the K8s cluster.
...
You can now receive alerts for Data Collection link:/task_system_monitors.html#collector-monitors[events]:

* Acquisition Unit Shutdown
* Collector Failed
* Collector Warning

Note that these Monitors are _Paused_ by default and must be enabled (resumed) before they will send alerts. 

Also note that in the case of the Collector Warning, this monitor can sometimes send more alerts than other alert types, so it is recommended to set no alert recipients on this monitor unless you are troubleshooting.



==== Alert on Change Log Events

Receive informational alerts for ONTAP Change Log events:
...
* *Cohesity Smart Files* - This REST API-based collector will acquire a Cohesity cluster, discovering the “Views” (as CI Internal Volumes), the various nodes, as well as collecting performance metrics. 
