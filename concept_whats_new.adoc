...
=== Improved Operator Installation
...
* Installation workflow provides more concise instructions and includes steps for uploading the operator images to your private repository.
* Common user configuration settings are held in a new config file which includes inline descriptions/documentation.
* Means of populating local/private/custom container repository with components necessary for Operator operation.
* Upgrades are now performed by installing over the previous install, requiring less user interaction.
...
=== Report Kubernetes PV data

Kubernetes persistent volume (PV) information collected by Cloud Insights is now available for use in Reporting.
Enabling reporting of K8S cluster from the physical host to the backend storage, for chargeback/showback and migrations.



=== Improved Workload Security screen loading

Cloud Insights has improved user experience by displaying Observability and Workload Security screens more efficiently, reducing load time as you navigate across these features. Note that Workload Security page URLs have changed. Previous URLs will automatically redirect to the new URLs.
