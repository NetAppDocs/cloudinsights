...
Cloud Insights now allows searching for attributes and metrics having null/none values through the use of filters. You can perform this filtering for both text-based and numeric attributes/metrics in the following places:
...
To filter for null/none values, simply select the italicized _None_ option when it appears in the appropriate filter drop-down.

image:Null_Filter_in_dropdown.png[Null filter in dropdown]
