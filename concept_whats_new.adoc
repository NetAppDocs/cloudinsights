...
In order to get the most out of Cloud Insights' ONTAP-Centric Dashboards, enable link:task_dc_na_cdot.adoc.html#a-note-about-permissions[*Advanced Counter Data Collection*] in the ONTAP data collector's *Advanced Configuration* section. Select all available Advanced Counters to ensure ONTAP Dashboard widgets display proper data.

[.thumb]
image:ONTAP_AdvancedCounterDataCollection.png[ONTAP Advance Configuration]
