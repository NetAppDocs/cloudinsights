...
The agent for ingestion of telegraf integration data has been updated to version 1.17.3, which includes vulnerability and bug fixes.

==== Add Corrective Actions to an Alert

You can now add an optional description as well as additional insights and/or corrective actions when creating ot modifying a Monitor by filling in the *Add an Alert Description* section. The insights/corrective action field can be up to 67,000 characters and will be displayed in the summary section of the alert landing page. The description will be sent with the alert. 

image:Monitors_Alert_Description.png[Alert Corrective Actions and Description]

==== Cloud Insights APIs for All Editions

APIs are available for Basic and Standard Editions of Cloud Insights, in addition to Premium. The categories of APIs available vary for each subscription level:

[cols="<,^s,^s,^s", role="green"]
|===
|API Category|Basic|Standard|Premium

|Acquisition Unit|icon:tags[role="blue"]|&check;|&check;
