...
=== Cloud Secure now works with ONTAP Autonomous Ransomware Protection

Ransomware protection is even better thanks to Cloud Secure integration with ONTAP’s link:concept_cs_integration_with_ontap_arp.html[Autonomous Ransomware Protection] (ARP). ONTAP ARP events are received by Cloud Secure, which takes the following actions:
...
•	Provides forensics to assist with data breach investigation
