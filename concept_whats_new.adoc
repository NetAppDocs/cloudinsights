...
Data Collector landing pages now include a list of recent changes. Simply click the "Recent Changes" button at the bottom of any data collector landing page to display changes occurring with that data collector within that last three weeks.
...
CIS-32027 Eliminate need to generate agent IDs server-side
...
These features are available now for integration-type data (ONTAP Advanced Metrics, etc.), and will be coming soon for Infrastructure objects (storage, volume, switch, etc.).
