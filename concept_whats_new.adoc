...
==== Cloud Secure Snapshot Purge and Data Collection Speed

Cloud Secure automatically deletes snapshots based on the Snapshot Purge Settings. Snapshot purge occurs once per day and only snapshots that age out are purged at that time.

image:CloudSecure_SnapshotPurgeSettings.png[Purge Settings]

Additionally, a single data collector agent system can post up to 20,000 events per second to Cloud Secure.
