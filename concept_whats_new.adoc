...
Two new widgets are available for displaying single-value data on your dashboards in eye-catching colors based on thresholds you specify. You can display values using either a *Solid Gauge* or *Bullet Gauge*. Values that land inside your Warning range are displayed in amber. Values that land in your Critical range are displayed in red. In Gauge widgets, values below Warning threshold are displayed in green.

In addition, you can now choose to display the Single-Value widget with a colored background based on Warning and Critical thresholds.
