...
Reporting a data collector error is easier with the "Error Report" button on the data collector page. Clicking the button sends basic information about the error to NetApp and prompts investigation into the problem. Once pressed, the Ui presents an acknowledgment that NetApp has been notified, and the Error Report button is disabled until the browser page is refreshed.
