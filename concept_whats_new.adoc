...
Dashboard grouping allows you to better link:concept_dashboard_features.html#dashboard-groups[view and manage the dashboards] that are relevant to you. You can add related dashboards to a group for "one-stop" management of, for example, your storage or virtual machines. 
...
You can also now pin dashboards within your groups, so favorite dashboards always appear at the top of the list. Pinning a dashboard in one group also pins it to the top of any other grouop containing that dashboard. Dashboards you pin in your groups are not pinned in another user's groups.

image:DashboardGroupsAndPins.png[Dashboard Groups and Pins]
