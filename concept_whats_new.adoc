...
==== Type-Ahead, Wildcards, and Expressions in Filters

When you are filtering for text or list values in queries or dashboard widgets, as you begin typing you are dynamically presented with a list of results from which to choose, as well as the option to create a *link:task_create_query.html#more-on-filtering[wildcard filter]* based on the current text. Selecting this option will return all results that match the wildcard expression. 

image:Type-Ahead-Example-ingest.png[Wildcard Filter]

You can also create *expressions* using NOT or OR, or you can select the "None" option to filter for null values in the field. 
