...
=== Alert when a log event occurs multiple times over a specified time

When creating a log monitor, you now have the ability to trigger an alert if an event occurs N times over a period of time you specify (minutes, hours, or days). 


=== Receive notifications when API tokens are about to expire

Cloud Insights will now send out notifications when API tokens are approaching expiration. Recipients will receive a notification when the token will expire in 30 days or fewer, and again when expiration is imminent. You must rotate expired tokens; learn more link:/API_Overview.html#rotating-expired-api-access-tokens[here].
