...
=== Removal of references to "Standard Edition" - this list TBD:

Banners (for example, Prompting to upgrade from Basic, API Screen on Trial Edition, etc.) will prompt users to upgrade to Premium only (i.e. no mention of "Standard")
Subscription table will remove Standard feature column
Metering: Total MU Usage and "Usage By Module" for Infrastructure Observability, Kubernetes Observability, Workload Security (TBD), including (more visible) MU estimators for each



=== Kubernetes Network Monitoring and Map is available for all

The link:concept_kubernetes_network_monitoring_and_map.html[_Kubernetes Network Performance and Map_] simplifies troubleshooting by mapping dependencies between Kubernetes workloads, providing real-time visibility into Kubernetes network performance latencies and anomalies to identify performance issues before they affect users. After a successful Preview, it's now available for all customers. 


== Reporting 

=== The Reporting component of Cloud Insights has been re-named to "Cloud Insights Analytics" 
...
Data Domain and Cohesity MUs metered at 40:1 (Expected June deployment)
RHEL and Rocky 9.0 and 9.1 support


=== Manually Install AU

TBD - Wait for UI changes before documenting this?


=== ONTAP Essentials dashboards going GA

Security Dashboard moving to GA
Data Protection Dashboard moving to GA:

Includes UI: Local Protection Overview
Includes UI: Remote Protection Overview
...
Qtree full path information added to 'pod-to-storage' when available (either set Manually or via Trident) (details TBD)
