...
=== No more stale alerts

<<<<<<< HEAD
Log alerts are often resolved based on criteria such as follow-up log events. But sometimes the subsequent event never happens, for example if EMS events (such as "Antivirus Server Busy") do not have a follow-up event indicating that the issue has been resolved, or because evicted pods no longer exist. In those cases, triggered alerts might remain in "active" state indefinitely.
...
=======
Log alerts are often resolved based on criteria such as follow-up log events. But sometimes the subsequent event never happens, for example if an EMS event (such as "Antivirus Server Busy") does not have a follow-up event indicating that the issue has been resolved, or because evicted pods no longer exist. In those cases, triggered alerts might remain in "active" state indefinitely.

Log alerts can now be resolved based on time; if the alert condition stops occurring, Cloud Insights can resolve the alert automatically after a specified time has passed. You can choose to resolve the alert in minutes, hours, or days.

image:alerts_resolve_based_on_time.png[Drop-down showing the option to resolve an alert automatically choosing the number 7 and the time period of Days]
>>>>>>> f853f6137ec5487d1291b34bd7695db4ed1ae6da
...
Ensuring accessibility to your data is essential, so we've made .CSV export image:csv_export_icon.png[.csv export icon] available for all metric queries, dashboard table widgets, and object landing pages, regardless of the type of data (asset or integration) you're querying. 
