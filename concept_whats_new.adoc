...
Customizable link:task_create_webhook.html[Webhooks] allow Cloud Insights to send alert notifications to various applications that support webhook channels.

Many commercial applications support webhooks as a standard input interface. Cloud Insights supports many of these delivery channels, providing default templates for Slack, PagerDuty, Teams, and Discord, in addition to providing customizable generic webhooks to support many other applications.
