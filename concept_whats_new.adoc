...
As part of a continuing effort to improve efficiency of data-gathering from ONTAP systems, the following ONTAP metrics have been renamed. 

If you have existing dashboard widgets using any of these metrics, you will need to edit or re-create those widgets to use the new metric names.
...
==== ONTAP Metrics Renamed

As part of an on-going effort to improve efficiency of data-gathering from ONTAP systems, the following ONTAP Metrics have been renamed.

|===
|Previous Metric Name|New Metric Name
|netapp_ontap.system_node.total_data |netapp_ontap.system_node.total_throughput 
|netapp_ontap.aggregate.total_transfers |netapp_ontap.aggregate.total_iops 
|netapp_ontap.nic_common.total_bytes |netapp_ontap.nic_common.total_throughput 
|netapp_ontap.lif.total_data |netapp_ontap.lif.total_throughput 
|netapp_ontap.volume.total_ops |netapp_ontap.volume.total_iops 
|netapp_ontap.volume.avg_latency |netapp_ontap.volume.total_latency 
|netapp_ontap.workload.ops |netapp_ontap.workload.total_iops 
|netapp_ontap.workload.total_data |netapp_ontap.workload.total_throughput 
|netapp_ontap.workload.latency |netapp_ontap.workload.total_latency 
|netapp_ontap.workload_volume.ops |netapp_ontap.workload_volume.total_iops 
|netapp_ontap.workload_volume.total_data |netapp_ontap.workload_volume.total_throughput 
|netapp_ontap.workload_volume.latency |netapp_ontap.workload_volume.total_latency 
|===

Dashboard widgets or monitors configured with any of these metrics will stop showing data as of the point in time where the data collector was updated. To use the new metrics, edit each dashboard widget or monitor and consult the table above to change the metric name to the new name.  
