...
Workload Security has been improved with the following enhancements:

* *Restricting User Access*: When a ransomware attack is detected, Workload Security can stop the attack by blocking user access to the file system using Automated Response Policies, or manually from the alert or user details pages. - link:/cs_restrict_user_access.html[Learn More]
* Whitelisting *Allowed Files Types*: If a ransomware attack is detected for a known file extension, that file extension can be whitelisted to prevent unnecessary alerting. - link:/ws_allowed_file_types.html[Learn More]
