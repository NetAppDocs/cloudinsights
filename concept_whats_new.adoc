...
[#whats-new]
[.lead]
NetApp is continually improving and enhancing its products and services. Here are some of the latest features and functionalities available in the Data Infrastructure Insights.  
=======
...
Track every configuration change and correlate it with your infrastructure's health and performance. Surface potential configuration changes that may be causing health issues. Simplify and reduce troubleshooting time from hours to minutes.

image:Change_Analysis_Example_showing_alert-change_correlation.png[Infrastructure Change Analytics example,900]


=== Arithmetic Expressions in Monitors

Expressions enable you to perform arithmetic operations on metrics, such as dividing one metric by another to determine percentages or ratios. Metric and Anomaly Detection Monitors now allow you to add a custom expression to the monitor query, giving you more freedom to alert on the data and situations you need.
...
Navigation between dashboards is smoother than ever, with the introduction of the dashboard navigation drop-down. Simply open the drop-down in the top "breadcrumb" trail and select another dashboard in the same group as the current dashboard, or--if the dashboard belongs to multiple groups--select a dashboard in any other  group to which the current dashboard belongs. You'll take fewer trips to the "All Dashboards" page to find what you're looking for.
...
Cloud Insights now has an updated object iconography, reducing overall variance of  icons used on the various Cloud Insights pages as well as grouping similar objects together (for example, Volume-related objects). Previous 'letter-based' icons are replaced with clear and concise graphics, making Cloud Insights screens more user-friendly and accessible.

Unsure of what an icon means? The table below (also available on the link:reference_object_icons.html[Object Icon Reference] page) explains them all.
