...
*link:concept_dashboard_features.html#auto-refresh-and-tv-mode[TV Mode]* gives you an uncluttered, *Auto-Refreshing* display for constant, near-real-time viewing of any dashboard or asset landing page. Data in widgets and tables is automatically refreshed according to the dashboard or landing page time range selected.
...
Auto-Refresh occurs for every widget on a dashboard based on the chosen interval, regardless of whether that widget is configured to override dashboard time. Auto-Refresh is available whether TV Mode is active or not, and can be paused and resumed at any time. Landing page charts and tables are all subject to Auto-Refresh according to the page time range chosen.

With TV Mode enabled, the following changes occur:

* The left-side Cloud Insights navigation menu is hidden.
* A button is displayed in the upper-left corner of the display that, when clicked, disables TV Mode and returns the navigation menu.
