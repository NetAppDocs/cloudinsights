...
It is easier than ever to create a copy of a widget on a dashboard or landing page. In dashboard Edit mode, click the "three dot" menu on the widget and select *Duplicate*. The widget, with its formatting and its underlying query/queries will be copied and placed at the bottom of your dashboard. You can then modify any parameters or formatting, rename it, and move it as needed.
