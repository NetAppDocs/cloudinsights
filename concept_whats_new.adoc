...
When you are filtering for text or list values in queries or dashboard widgets, as you begin typing you are presented with a list of results from which to choose, as well as the option to create a *link:task_create_query.html#more-on-filtering[wildcard filter]* based on the current text. Selecting this option will return all results that match the wildcard expression. 
