...
Cloud Insights collects *Risk* data from NetApp Clustered Data OnTap storage systems that are monitored by link:concept_activeiq.html[NetApp Active IQ], which provides a series of visualizations, analytics, and other support-related services to NetApp customers for their hardware / software systems. You do not need to configure Cloud Insights to collect this information; it is collected automatically for any Active IQ-monitored Clustered Data OnTap storage system in your Cloud Insights environment.
