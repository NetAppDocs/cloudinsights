...
In addition to API categories for managing Assets and Data Collection, the following additional API category is available:

* *Data Ingection*: Used to retrieve and manage ingestion data, such as from ONTAP Advanced Metrics, Kubernetes, or Docker

For environments with Cloud Insights Reporting, the *Data Warehouse* category is also available.
