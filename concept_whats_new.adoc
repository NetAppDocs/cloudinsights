...
Cloud Insights now filters for Null values on the Query page, in Dashboard Widgets, on the Alerts list page, and when creating Monitors. To filter for Null values, simply select the _None_ option when it appears in the appropriate filter drop-down. 
