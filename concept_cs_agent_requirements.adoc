...
Note that when adding _<<task_add_collector_svm.html#permissions-when-adding-via-cluster-management-ip, csuser>>_, that user requires SSH access to the ONTAP management LIF.
...
|TCP|7|SVM data LIF IP Addresses|Bidirectional |Bidirectional between ONTAP and Cloud Secure. Agent pings the SVM Lifs.
|===
