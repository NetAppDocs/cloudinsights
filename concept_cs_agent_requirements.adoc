...
NOTE: CloudInsight agent and CloudSecure agent can be installed in the same machine. However, it is a best practice to install the 2 in separate machines.In case both the agents are installed in the same machine, please allocate more diskspace as is shown below.
+
|===
