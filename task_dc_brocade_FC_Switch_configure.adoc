...
= Configuring the Cisco MDS Fabric Switches data collector
...
Cloud Insights uses the Cisco MDS Fabric Switches data collector to discover inventory for Cicso or rebranded switch devices running Factored Operating System (FOS) firmware 4.2 and later. Devices in both FC switch and Access Gateway modes are supported. 
