...
|Infrastructure, Storage, VM Metrics|NetApp Data Collectors Only|All Available Data Collectors
|Real-Time Dashboards|&check;|&check;
|Real-Time Inventory Search|&check;|&check;
|Metrics from Over 50 Services|X|&check; 
|Dynamic Alerting (Monitoring)|X|&check; 
|===
