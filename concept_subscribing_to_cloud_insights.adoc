...
Allow us to provide a CI entitlement to a customer via a foreign serial number. Example - Astra sells a license, the customer gets the Astra license S/N, CI is bundled with the Astra deal, hence the Astra license S/N is what is used to identify the subscription in Zuora. Allow the customer to add additional entitlement ID's via the CI UI and use these to match subscriptions and pull them into the tenant.
