...
|User Management|image:SmallCheckMark.png[]|image:SmallCheckMark.png[]|image:SmallCheckMark.png[]
...
Additionally, your Cloud Insights link:https://docs.netapp.com/us-en/cloudinsights/concept_user_roles.html#permission-levels[feature set role] will determine which APIs you can access. User and Guest roles have fewer privileges than Administrator role. For example, if you have Administrator role in Observability, but User role in Reporting, you can manage all API types except Data Warehouse.
...
image:APICategoriesApril2022.png[API Categories] 
...
Note that the API categories to which a user has access depend also on the link:concept_user_roles.html[User Role] they have in each Cloud Insights feature set (Observability, Security, Reporting).
