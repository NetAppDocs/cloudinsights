...
If your system uses a proxy, the Acquisition Unit uses 2-way/mutual authentication to connect to the Cloud Insights server, which means the client certificate must be passed all the way to the Cloud Insights server in order to be authenticated. To accomplish this, the proxy should be set up to forward the http request to the Cloud Insights server without decrypting the data.
