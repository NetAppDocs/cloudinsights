...
Cloud Insights and macOS report different numbers for memory usage. Both telegraf and the Mac activity monitor use metrics gathered from _vm_stat_, however the total memory usage is calculated differently for each. 

Telegraf calculates _Memory Used Total_ as follows, where _Memory Available Total_ is derived from the sum of "Pages free" and "Pages inactive" in _vm_stat_: 

 Memory Used Total = Memory Total - Memory Available Total 

Mac's activity monitor, on the other hand, calculates Memory Used as follows: 

 Memory Used = App Memory + Wired Memory + Compressed 

Where _App Memory_ is derived from the difference between “Anonymous pages” and “Pages purgeable” in _vm_stat_,  
