...
* Access validation for performance: In a web browser on your Acquisition Unit, go to _https://<SMI-S Hostname or IP>:5989/ecomconfig_ where "SMI-S Hostname or IP" is the IP address or hostname of your SMI-S server. This URL is for an administrative portal for the EMC SMI-S (aka "ECOM") service - you will receive a login popup.   
