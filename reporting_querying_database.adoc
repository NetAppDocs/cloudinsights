...
By default, an API command will operate in _synchronous_ mode, meaning that you send the request and the respoonse is returned immediately. However, at times a query may take a long time to execute, which could lead to timing out. To get ariund this, you can execute a request _asynchronously_. In asynchronous mode, the request will return a URL through which the execution can be monitored. The URL will retrun the result when it is ready.

To execute a query in async mode, add the header "_Prefer: respond-async_" to the request. Upon successful execution, the response will contain the following headers:

 Status Code: 202 (which means ACCEPTED)

 preference-applied: respond-async

 location: https://<domain>/rest/v1/dwh-management/odata/dwh_custom/asyncStatus/<token>

Querying the location URL will return the same headers if the response is not ready yet, or will return the response with status 200 if the response is ready, along with additional header information as noted below. The status in the header indicates the status of the original query.
...
To see a list of all async queries and which of them are ready, use the command _GET https://<ci_environment>/rest/v1/dwh-management/odata/dwh_custom/asyncList
...
The response has the following format:

 {
    "queries" : [
        {
            "Query": "https://<ci_environment>/rest/v1/dwh-management/odata/dwh_custom/heavy_left_join3?$count=true",
            "Location": "https://<ci_environment>/rest/v1/dwh-management/odata/dwh_custom/asyncStatus/<token>",
            "Finished": false
        }
    ]
 }
