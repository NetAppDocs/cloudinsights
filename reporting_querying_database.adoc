...
Querying the location URL will return the same headers if the response is not yet ready, or it will return status 200 without the location header if the response is ready. The response content will contain the JSON result of the original query, with an addition field named `queryStatus`, which is the HTTP status result of the original query.

....
{
    "queryStatus":200,
    "@odata.context":"https://<instance>.cloudinsights.netapp.com/rest/v1/dwh-management/odata/<context>",
    "value":[<data>]
}  
....
