...
NOTE: This information is considered Preview documentation and is subject to change.

This data collector acquires from the *HCI Virtual Center only*. To collect data from the storage system, you must also configure the link:task_dc_na_storagegrid.html[NetApp StorageGrid] data collector.
...
* We require read only privileges on all objects within the HCI Virtual Center. 
* SDK access on the HCI Management server – normally already setup. 
* Port requirements: http-80 https-8443 
* Validate access: 
** Log into HCI Management server using above username and password 
** Verify SDK enabled: telnet <vc_ip> 8443 
...
|Error: Failed to instantiate a connection to Virtual Center at IP
|Possible solutions:

* Verify credentials and IP address entered.
* Try to communicate with Virtual Center using HCI Management Infrastructure Client.
...
* Recommended: Re-generate certificate for HCI Virtual Center by using stronger (e.g. 1024-bit) RSA key.
