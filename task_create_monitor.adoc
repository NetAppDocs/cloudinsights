...
. In the drop-down, search for and choose an object type and metric (for Metric Monitor) or Log (for Log Monitor) to monitor.
...
image:Monitor_log_monitor_choose.png[choose which log to monitor]
...
NOTE: You cannot modify the conditions of a system-defined monitor. To change the conditional, first save the monitor with a different name (Save As), and then modify the new monitor.

You _can_ update the Notifications section of a system-defined monitor.
