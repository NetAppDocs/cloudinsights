...
[.lead]
You create monitors to set thresholds that trigger alerts to notify you about issues related to the resources in your network. For example, you can create a monitor to alert you when node write latency reaches a certain point.
...
In the example below, we will create a Monitor to give a Warning alert when NFS Write Latency reaches or exceeds 200ms, and a Critical alert when it reaches or exceeds 400ms. We only want to be alerted when either threshold is exceeded for at least 15 continuous minutes.

//image:MonitorExample1.png[Storage IOPS Monitor Example]

=== Create the Monitor 

. From the Cloud Insights menu, click *Alerts > Monitors*
+
