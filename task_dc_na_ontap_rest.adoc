...
* You must have access to an user account with the require level of access
* Account details include username and password.
* Port requirements: 443

* Account permissions:
** Functionally, Cloud Insights is making read requests, but some write permissions are required for CI to register with the Ontap array. See the Note About Permissions below.
