...
** Functionally, Cloud Insights primarily makes read requests, but some write permissions are required for Cloud Insights to register with the ONTAP array. See the _Note About Permissions_ immediately below.
...
Storage Virtual Machines: REST no longer reports SVM's not of type 'data' (like 'cluster' or 'mgmt' or 'node').
