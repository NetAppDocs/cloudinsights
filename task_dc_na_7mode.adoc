...
For storage systems using Data ONTAP software operating in 7-Mode, you should use the ONTAPI data collector, which uses the CLI to obtain performance and capacity data. 
...
To acquire a 7-Mode data collector successfully and generate no warnings on the storage system, you should use one of the following command strings to define your user roles. The second string listed here is a streamlined version of the first:
