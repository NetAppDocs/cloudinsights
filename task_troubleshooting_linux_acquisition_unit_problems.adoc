...
|When installing a new Cloud Insights Acquisition Unit it fails to connect.
|1. If a proxy server is needed you may need to uninstall and reinstall the AU with the correct proxy settings.  
2. Ensure that the domain _*.cloudinsights.netapp.com_ is included in your proxy exception list. More informaiton can be found in the link:https://kb.netapp.com/Advice_and_Troubleshooting/Cloud_Services/Cloud_Insights/Cloud_Insights_Acquisition_Unit_fails_to_connect_for_new_install[NetApp Knowledgebase]. 
|===
