...
NOTE: When using the Amazon FSx data collector, user blocking for SMB cannot kill the current user session.  See the <<troubleshooting, Troubleshooting>> section for more information.
...
|I see an error:
svm1: existing cifs session for user domain\\user01 is not closed.

This error is seen in the Action Taken section on the alert detail page and also under Access Limitation History under Alert & User list pages.

The users current session is not closed when this error is seen, however, user is blocked for any new sessions until the bocking period expires.
|This is a known issue with Amazon FSx. Cloud Secure will NOT be able to close the existing SMB session. No workaround is currently available to block the existing SMB session for Amazon FSx.

If the collector type is CVO or ONTAP, ensure the correct permissions as mentioned in the <<prerequisites-for-user-access-blocking,Prerequisites>> section.
