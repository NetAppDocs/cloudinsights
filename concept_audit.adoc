...
| Not * |As with _None_ above, but you can use this form to search for NULL values only in text fields | Not * |returns results where the target field is not empty
| "" |treats everything between the first and last quote as an exact match| "NetApp_*" | returns results containing the exact literal string _NetApp\_*_
|===
