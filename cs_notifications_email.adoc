...
= Configuring Email Notifications for Alerts, Warnings, Agent, and Data Source collector health
...
[.lead]
To configure alert recipients, click on *Admin > Notifications* and enter an email addresses in the appropriate section(s) for each recipient.
...
* Active directory collection failure is reported as a warning, as it does not impact Ransomware detection.
