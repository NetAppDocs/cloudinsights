...
When filtering by CLUSTER, use the CI-Style cluster names as reported by CI on hypervisors - CI prepends the vSphere cluster name with the vSphere datacenter name and a backslash - "DC1\clusterA" is the cluster name CI would report on a hypervisor in clusterA within data center DC1
