...
* In multi-tenant Kubernetes environments, outages may happen because of mis-configured changes. In very dynamic environments, Cloud Insights infrastructure monitoring may not be able to properly track all changes.
* Change Analytics helps with this by providing a single pane to view and correlate the health of workloads and configuration changes. This may help in troubleshooting dynamic Kubernetes environments.
