...
When you are filtering for text or list values, as you begin typing you are presented with ther option to create a wildcard filter based on what you are typing. Selecting this option will return all results that match the wildcard expression. Filters based on wildcards or expressions (e.g. NOT, OR, "None", etc.) display in dark blue in the filter field. 
