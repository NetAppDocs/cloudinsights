...
When you are filtering for text or list values, as you begin typing you are presented with ther option to create a *wildcard filter* based on the current text. Selecting this option will return all results that match the wildcard expression. You can also create *expressions* using NOT or OR, or you can select the "None" option to filter for null values in the field.
...
Note that Wildcard and Expressions work with text or lists but not with numerics or dates.
