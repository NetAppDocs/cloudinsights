...
| * (Asterisk) |enables you to search for everything | vol*rhel |displays all resources that start with "vol" and end with "rhel"
| ? (question mark) |enables you to search for a specific number of characters|  BOS-PRD??-S12 |displays BOS-PRD12-S12, BOS-PRD13-S12, and so on
| OR |enables you to specify multiple entities | FAS2240 OR CX600 OR FAS3270 |finds multiple storage models
| NOT |allows you to exclude text from the search results |  NOT EMC* |finds everything that does not start with "EMC"
